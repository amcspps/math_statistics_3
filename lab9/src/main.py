...
def draw_intervals(interval_data, folder=".", ext="pdf"):
    ...
    for i in range(len(interval_data)):
        plt.vlines(i, interval_data[i].low, interval_data[i].high, colors="g", lw=1)
...
def draw_intervals_including_mode(interval_data, mode, folder=".", ext="pdf"):
    plt.clf()
    for i in range(len(interval_data)):
        color = "g" if mode.inside(interval_data[i]) else "r"
        plt.vlines(i, interval_data[i].low, interval_data[i].high, colors=color, lw=1)

    plt.plot([0, len(interval_data)], [mode.low, mode.low], color="g", ls='--', lw=1)
    plt.plot([0, len(interval_data)], [mode.high, mode.high], color="g", ls='--', lw=1)
...
def draw_mode_frequencies(interval_data, mu, k, folder=".", ext="pdf"):
    ...
    plt.plot(x, mu)
    plt.vlines([interval_data[k[0]].low, interval_data[k[-1]].high], [0, 0], [max(mu), max(mu)], color='b', ls='--')
...
def draw_oskorbin_optimization_results(interval_data, omega, beta, folder=".", ext="pdf"):
    ...
    plt.clf()
    for i in range(len(interval_data)):
        plt.vlines(i, new_data[i].low, new_data[i].high, colors="y", lw=1)
...
data = load_csv("../data/Channel_1_700nm_0.23mm.csv")
